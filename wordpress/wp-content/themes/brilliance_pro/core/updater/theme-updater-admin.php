...
class EDD_Theme_Updater_Admin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'brilliance-registration-section', array( $this, 'license_page' ) );
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( false === $license_data->success ) {
				...
				if ( ! empty( $message ) ) {
					$base_url = admin_url( 'themes.php?page=cpotheme-welcome' );
					$redirect = add_query_arg( array( 'tab' => 'registration', 'sl_theme_activation' => 'false', 'message' => urlencode( $message ) ), $base_url );
		...
		wp_redirect( admin_url( 'themes.php?page=cpotheme-welcome&tab=registration' ) );
		exit();
