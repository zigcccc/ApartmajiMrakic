...
if(!function_exists('cpotheme_setup')){
	...
	function cpotheme_setup(){
		...
		//require( get_template_directory() . '/core/updater/theme-updater.php' );

		// Backward compatibility for license key
		$old_license_key = get_option('license_'.CPOTHEME_ID);
		if ( $old_license_key ) {
			update_option( 'brilliance_pro_license_key', $old_license_key );
			delete_option( 'license_'.CPOTHEME_ID );
		}
		...
		if ( !$epsilon ) {
			$req_plugins = get_option( 'brilliance_show_recommended_plugins' );
			...
			if ( !empty( $req_plugins ) ) {
				...
				update_option( 'brilliance_show_recommended_plugins', $updated_req_plugins );
			}
