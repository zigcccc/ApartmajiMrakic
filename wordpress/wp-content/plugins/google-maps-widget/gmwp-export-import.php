...
class GMWP_export_import {
  ...
  static function send_export_file() {
    ...
    $filename .= '-' . date('Y-m-d') . '-googlemapswidgetspro.txt';

    $out = array('type' => 'GMWP export', 'version' => GMWP::$version, 'data' => self::generate_export_data());
    $out = json_encode($out);
  ...
  static function validate_import_file() {
    ...
    if (!isset($content['type']) || !isset($content['version']) || !isset($content['data']) ||
        $content['type'] != 'GMWP export' || !is_array($content['data'])) {
      return new WP_Error(1, 'Uploaded file is not a GMWP export file. Please verify that you have uploaded the right file.');
    }
